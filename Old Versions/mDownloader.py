...
def User():
    ...
    global seriesName
    global driver
    global number_of_latest_volume

    seriesName = input('The name of the series: ')
    mainLink = input('The main Link: ')
    number_of_latest_volume = int(input('The number of the latest volume: '))
...
def downloadImages():
    ...
    if fCounter < 10:
        os.system('mkdir Vol.00%s' %(number_of_latest_volume))
        os.chdir('Vol.00%s' %(number_of_latest_volume))

    elif fCounter >= 10 and fCounter < 100:
        os.system('mkdir Vol.0%s' %(number_of_latest_volume))
        os.chdir('Vol.0%s' %(number_of_latest_volume))

    elif fCounter >= 100:
        os.system('mkdir Vol.%s' %(number_of_latest_volume))
        os.chdir('Vol.%s' %(number_of_latest_volume))
    ...
    if fCounter < 10:
        os.system('convert *.JPEG "%s-Vol.00%d.pdf"' %(seriesName,number_of_latest_volume))
        os.system('mv "%s-Vol.00%d.pdf" %s' %(seriesName,number_of_latest_volume,pdfDirectory))

    elif fCounter >= 10 and fCounter < 100:
        os.system('convert *.JPEG "%s-Vol.0%d.pdf" ' %(seriesName,number_of_latest_volume))
        os.system('mv "%s-Vol.0%d.pdf" %s' %(seriesName,number_of_latest_volume,pdfDirectory))

    elif fCounter >= 100:
        os.system('convert *.JPEG "%s-Vol.%d-.pdf"' %(seriesName,number_of_latest_volume))
        os.system('mv "%s-Vol.%d-.pdf" %s' %(seriesName,number_of_latest_volume,pdfDirectory))

    #Decreases the fCounter number to keep track of the volume it's creating(it downloads from the newest to the oldest)
    fCounter-=1
    number_of_latest_volume-=1
